...
const int minPressureTo   = 350; // Minimal Pressure set To

const int maxPressureFrom = 250; // Maximum Pressure set From
const int maxPressureTo   = 420; // Maximum Pressure set To
...
//Serial.println("connected");
      client.subscribe("yard/waterSupply/pumpControl");
    } else {
...
void publishPressure()
{
  ...
  if (time - _prevTime > 1000 && checkConnect() && abs(pressure - _prevPressure) > 10) {
    ...
    client.publish("yard/waterSupply/pressure", buffer);
...
void setErrorMode(int m_errorCode)
{
  ...
  if (mode != 5 || m_errorCode > errorCode) {
    ...
    if (checkConnect()) {
      ...
      client.publish("yard/waterSupply/error", buffer);
    }
  }
}
// ------ Change Mode ------
  ...
  if (mode != m_mode) {
    ...
    switch (mode) {
      case 0:
        ...
        redVal   = HIGH;
        if (checkConnect()) {
          client.publish("yard/waterSupply/pumpStatus", "stop");
        }
        break;
      case 1:
        greenVal = HIGH;
        if (checkConnect()) {
          client.publish("yard/waterSupply/pumpStatus", "wait");
        }
        break;
      case 2:
        blueVal = HIGH;
        pumpVal = HIGH;
        if (checkConnect()) {
          client.publish("yard/waterSupply/pumpStatus", "pumping");
        }
        break;
      case 4:
        redVal   = HIGH;
      ...
      case 5:
        redVal = HIGH;
        if (checkConnect()) {
          client.publish("yard/waterSupply/pumpStatus", "error");
        }
        break;
    }
    if (mode != 5) {
